...
Waypoint is a GTK/libadwaita snapshot and rollback tool with a built in scheduling service for Btrfs filesystems on Void Linux. For a complete walkthrough, see the [User Guide](docs/USER_GUIDE.md). For advanced users, waypoint-cli is available and has full feature parity with the GUI application.
