...
pub fn show_scheduler_dialog(parent: &adw::ApplicationWindow) {
    ...
    save_btn.connect_clicked(move |_| {
        ...
        // Validate snapshot prefix
        if let Err(error) = waypoint_common::validation::validate_snapshot_prefix(&prefix_str) {
            let error_dialog = adw::MessageDialog::new(
                Some(&dialog_for_save),
                Some("Invalid Prefix"),
                Some(&format!("Snapshot prefix is invalid:\n\n{}", error))
            );
        ...
        // Validate time format (should always be valid from spin buttons, but check anyway)
        if let Err(error) = waypoint_common::validation::validate_time_format(&time_str) {
            let error_dialog = adw::MessageDialog::new(
                Some(&dialog_for_save),
                Some("Invalid Time"),
                Some(&format!("Time format is invalid:\n\n{}", error))
            );
            error_dialog.add_response("ok", "OK");
            error_dialog.set_default_response(Some("ok"));
            error_dialog.present();
